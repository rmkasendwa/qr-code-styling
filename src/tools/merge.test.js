...
describe('Test getMode function', () => {
  ...
  it('Merge two objects with arrays', () => {
    expect(mergeDeep(objectWithArray, { arr: [3, 4] })).toEqual({
      arr: [3, 4],
    });
  });
  it('Merge two objects with nested objects', () => {
    expect(mergeDeep(nestedObject, { obj: { bar: 'bar' } })).toEqual({
      obj: { foo: 'foo', bar: 'bar' },
    });
  });
  it('Merge three objects with nested objects', () => {
    expect(
      mergeDeep(nestedObjectWithArray, nestedObject, { obj: { arr: [3, 4] } })
    ).toEqual({
      obj: {
        foo: 'foo',
        arr: [3, 4],
