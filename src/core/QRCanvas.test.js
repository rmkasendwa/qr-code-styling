...
describe('Test QRCanvas class', () => {
  ...
  beforeAll(() => {
    qr = qrcode(
      defaultOptions.qrOptions.typeNumber,
      defaultOptions.qrOptions.errorCorrectionLevel
    );
  ...
  it('Should draw simple qr code', () => {
    const expectedQRCodeFile = fs.readFileSync(
      path.resolve(__dirname, '../assets/test/simple_qr.png'),
      'base64'
    );
    const canvas = new QRCanvas(defaultOptions);

    canvas.drawQR(qr);
    expect(canvas.getCanvas().toDataURL()).toEqual(
      expect.stringContaining(expectedQRCodeFile)
    );
  });
  ...
  it('Should draw a qr code with margin around canvas', () => {
    const expectedQRCodeFile = fs.readFileSync(
      path.resolve(
        __dirname,
        '../assets/test/simple_qr_with_margin_canvas.png'
      ),
      'base64'
    );
    const canvas = new QRCanvas({
